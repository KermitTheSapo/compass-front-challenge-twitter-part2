...
export default function Main({ toggleTheme }: Props) {
  ...
  const checkLoginAndCreatePost = (e: Event) => {
    e.preventDefault()
    if (isGuess === true) {
      alert("You are in guest mode, you can't post")
      return;
  ...
  return (
    <S.Main>
      ...
      <S.Post>
        <S.post__form
          onSubmit={(e) => { checkLoginAndCreatePost(e) }}
          method="post"
        >
          <S.post__input>
            <S.input__img
            ...
            />
            <S.input__text id="input" ref={inputElement} type="text" placeholder="What's happening?" minLength={2} maxLength={380} value={inputValue} onChange={(event) => { changeOpacity(event) }} />
          </S.post__input>
          <S.post__emoji>
            <S.emoji__div>
              ...
              <S.upload__img id="upload" type="file" name="file" accept="image/png, image/jpeg" multiple onChange={(event) => { getImage(event) }} />
              <S.icons onClick={() => { checkLogin() }} src={gifIcon} alt="A square with 'GIF' inside" />
              <S.icons onClick={() => { checkLogin() }} src={graphIcon} alt="A chart icon" />
              <S.icons onClick={() => { checkLogin() }} src={emojiIcon} alt="Happy face icon" />
              <S.icons onClick={() => { checkLogin() }} src={CalendarIcon} alt="Calendar icon" />
            </S.emoji__div>
